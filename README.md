...
5. 切到抽奖屏后点「开始抽奖」滚动（仅抽奖屏可点开始；参与/奖品/中奖屏该按钮置灰）。滚动名单为尚未中奖的人。点「停」开奖；约 40ms 内停到中奖人并高亮，再停留约 3 秒（此期间下拉框和抽奖屏仍是刚抽的这一奖）。同一奖未抽满则继续开始/停；抽满后自动进中奖公示（上方仍是刚抽完这一奖），这时控制条当前奖才变为下一个未抽完的奖。再切到抽奖屏即可抽下一奖。全部抽完后按钮为「已抽取」。
