...
- 参与者默认名单在 `data/participants.xml`；当场 id、内定、中奖在 `data/*.json`。关闭浏览器不影响当场数据。**重启服务进程**会按 XML 开新一场（中奖与内定清空）。
...
本机默认名单在 `data/participants.xml`（只存姓名）。服务器**进程启动**时会按这份 XML 重建 `data/participants.json`（每人新的内部 id），并清空中奖记录与内定，大屏回到「参与」屏。刷新浏览器不会重开一场。

现场在参与屏添加、改名、删除（以及后台清空全部参与者）会把当前姓名写回 XML，下次启动仍用这份名单。

## 现场流程

1. 打开大屏 URL，投屏给观众。新会话默认停在「参与」屏。
2. 用控制条切换：参与 → 奖品 → 抽奖 → 中奖；可「隐藏」控制条。
3. 启动后参与页先显示 XML 里的默认名单；可继续逐个添加、改名、删除（已中奖者不能删除；重名会提示）。无需填写用户 ID。
4. 内定仍在 `/admin` 按次设置（可选）。奖品名称、数量、图片用上面的本地命令改。
5. 在奖品页或抽奖页点「开始抽奖」滚动（参与页该按钮置灰。同一奖未抽满时，提前进入中奖页也可开始。一奖抽满后的中奖页公示刚抽完的奖、下拉框已是下一奖，此时开始按钮仍置灰，须先切到奖品页或抽奖页再开下一奖）。滚动名单为尚未中奖的人。点「停」后再滚约 40ms，停在当时中间格的人并高亮；这个人写入中奖记录并在中奖页公示；管理页内定不在点停时生效。再停留约 3 秒（此期间下拉框和抽奖屏仍是刚抽的这一奖）。同一奖未抽满则继续开始/停；抽满后自动进中奖公示（上方仍是刚抽完这一奖），这时控制条当前奖才变为下一个未抽完的奖。再切到奖品页或抽奖页即可抽下一奖。全部抽完后按钮为「已抽取」。
